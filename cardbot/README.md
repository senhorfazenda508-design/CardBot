...
## Chat Livre da IA (o bot participando do servidor como mais um membro)

Além dos NPCs do RPG, o **próprio bot** (CardBot) tem uma IA com personalidade própria que participa do servidor como se fosse mais um membro — não é um personagem do jogo, é a "voz" do bot de verdade.

- `/chatia ligar canal:#geral` (admin) ativa o chat livre num canal: a partir daí, **toda mensagem normal** ali vira conversa com o bot, sem precisar de comando nem @menção.
- Fora de um canal ligado, o bot sempre responde se for **@mencionado** diretamente, em qualquer canal — e sempre responde em **DM**.
- `/chatia pausar canal:#geral` desliga um canal específico; `/chatia pausartudo` é o botão de pânico (desliga tudo de uma vez no servidor); `/chatia status` mostra onde está ativo.

**O que ele consegue fazer, além de conversar:**
- 👀 **Enxergar imagens** mandadas no chat (fotos, prints, memes) — via OpenRouter, modelo com visão.
- 🎨 **Gerar imagens** a partir de uma descrição, quando alguém pede um desenho/arte/meme — via API de imagens da OpenRouter (`OPENROUTER_IMAGE_MODEL`, padrão `google/gemini-2.5-flash-image`), usando a mesma `OPENROUTER_API_KEY`.
- 📜 **Ler o canal** — pode puxar as últimas mensagens reais do canal atual pra se situar do que rolou antes ("resume o papo aqui", "sobre o que a gente tava falando?").
- 🃏 **Consultar seus dados de jogo** — moedas, cartas, vitórias/derrotas — só de quem está conversando com ele, igual ao `/perfil`, sem acessar dado de outra pessoa.
- 💻 **Programar** — quando alguém pede um script/código de verdade, ele escreve o código normalmente e o bot detecta automaticamente e manda também como **arquivo anexado de verdade** (não só um bloco de texto pra copiar).

Essas ferramentas rodam no cérebro da **Groq** (`GROQ_API_KEY`, modelo `openai/gpt-oss-120b` — suporta tool calling), decidindo sozinho quando vale a pena usar cada uma. Mensagens com imagem continuam indo pro modelo de visão da OpenRouter, como antes. Sem `GROQ_API_KEY` configurada, o chat livre não funciona (mesma chave já usada pelo Thomas/taverneiro); sem `OPENROUTER_API_KEY`, a geração de imagem e a visão ficam indisponíveis, mas o resto do chat continua normal.

Assim como no resto do bot, todas as regras de segurança (nada de discurso de ódio, conteúdo adulto, instruções perigosas, tentativa de "jailbreak" via chat) valem por cima de qualquer ferramenta nova — ver `REGRAS_FIXAS` em `game/chatBotIA.js`.
